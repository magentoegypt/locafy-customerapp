...
class PaymentMethods extends StatefulWidget {
  ...
  const PaymentMethods({super.key, this.onBack, this.onFinish, this.onLoading});
...
class _PaymentMethodsState extends State<PaymentMethods> {
  ...
  void placeOrder(PaymentMethodModel paymentMethodModel, CartModel cartModel) {
    //"sympl", "cashondelivery", "online", "wallet"
    if(selectedId == "sympl"){
      _processPayment(paymentMethodModel,cartModel,"sympl");
    }else if(selectedId == "online"){
      _processPayment(paymentMethodModel,cartModel,"online");
    }else if(selectedId == "aman"){
      _processPayment(paymentMethodModel,cartModel,"aman");
    }else {
      callCreateApi(paymentMethodModel, cartModel);
    }

  }

  Future<void> _processPayment(PaymentMethodModel paymentMethodModel, CartModel cartModel,String paymentMethod) async {
    widget.onLoading!(true);
    try {
      final result = await _paymobService.initiatePayment(
        amount: cartModel.getTotal() ?? 1, // $100
        currency: 'EGP',
        ...
        billingData: {
          ...
          "street": cartModel.address?.street,
          "building": cartModel.address?.street,
      ...
      if (result['success'] == true) {
            ...
            builder: (context) => PaymentWebview(
              ...
              onFinish: (success, message) {
                if(success){
                  ...
                  _showPaymentResult(success, message);
                }
              },
            ),
          ),
        ...
        _showPaymentResult(false, result['error'] ?? 'Unknown error');
      }
    } catch (e) {
      _showPaymentResult(false, 'Error: $e');
    } finally {
  ...
  void callCreateApi(PaymentMethodModel paymentMethodModel, CartModel cartModel){
