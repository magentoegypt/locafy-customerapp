...
class OrderHistoryDetailScreen extends StatefulWidget {
  ...
  const OrderHistoryDetailScreen(
      {super.key, this.enableReorder = true, this.disableReview = false});
...
class _OrderHistoryDetailScreenState
      ...
      return Scaffold(
        ...
        body: RefreshIndicator(
          ...
          child: SingleChildScrollView(
            // Short orders (a single line item) would not otherwise overscroll,
            // so the indicator would never fire.
  ...
  /// Pull-to-refresh: re-fetch everything [afterFirstLayout] loads, so an
  /// order whose status or note changed server-side updates in place.
  Future<void> _reload() async {
    await orderHistoryModel.getOrderNote();
    await _loadLocalizedCountryName();
  }
