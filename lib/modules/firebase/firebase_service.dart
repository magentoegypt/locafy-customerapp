...
class FirebaseServices extends BaseFirebaseServices {
  ...
  Future<void> init() async {
    ...
    // Pass the options explicitly instead of letting the native SDK read them
    // from res/values.xml.
    //
    // That resource is generated by the com.google.gms.google-services Gradle
    // plugin from android/app/google-services.json, and the plugin is not
    // applied here — so the lookup threw "Failed to load FirebaseOptions from
    // resource", init aborted before the line below, isEnabled stayed false,
    // and Services.getNotificationService() silently handed out the no-op
    // NotificationServiceImpl. The toggle then flipped to ON while nothing
    // subscribed to FCM (86d3fh4g7 #1).
    //
    // Deliberately NOT fixed by applying that Gradle plugin: the checked-in
    // google-services.json is for Firebase project "headless-31464", whereas
    // this app belongs to "magento-app-store" (see firebase_options.dart).
    // Applying it would initialise the native layer against the wrong project.
